...
# Allow cross-origin for /api/* and allow credentials (cookies)
...
def api_admin_upload_video():
    ...
    # If a client accidentally tries to do a single huge upload that exceeds MAX_CONTENT_LENGTH,
    # the server will already return 413 — but we can proactively check content-length and respond
    # with guidance to use the chunk endpoints.
    try:
    ...
    try:
        # Save file (Flask's FileStorage streams to disk; should not keep huge buffer in memory)
        f.save(path)
...
def api_admin_upload_video_chunk():
    ...
    try:
        ...
        logger.info("Saved chunk %s for upload %s", chunk_index, upload_id)
